...
-- also returns the length of the prefix satisfying a given predicate.
spanWithLength :: (a -> Bool) -> [a] -> (Int, [a], [a])
spanWithLength p = go 0
  where
    go !len [] = (len, [], [])
    go !len xs@(c:cs)
      | p c       = let (n, a, b) = go (len + 1) cs in (n, c:a, b)
      | otherwise = (len, [], xs)
