...
inferExpr path packages envs (Variable str tok) = do
    ...
    if str == "this" then inferThis path pos
        ...
        -- 优先通过 ContextCheck 记录�?VarId 查找本地变量�?
...
inferStmt path packages envs (DefineVar names rhs toks) = do
    tRhs <- inferExpr path packages envs rhs
    case names of
        [name] -> do
            c <- get
            ...
            case lookupVarId name cState of
                Just (vid, defPos) -> do
                    _ <- ensureVarFlags vid cState
                    recordVarUseLocal defPos name vid
                    case Map.lookup vid (tcVarTypes c) of
                        Nothing -> modify $ \c0 -> c0 { tcVarTypes = Map.insert vid (tRhs, defPos) (tcVarTypes c0) }
                        Just (oldT, _) -> checkTypeCompat path (map Lex.tokenPos toks) oldT tRhs
                Nothing ->
                    addErr $ UE.Syntax $ UE.makeError path (map Lex.tokenPos toks) (UE.undefinedIdentity name)
        _ ->
