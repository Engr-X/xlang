...
import Parse.SyntaxTree (AccessModified(..), Class(..), DeclFlag(..))
...
opPrefix (FuncPtr _ _) = error "opPrefix: function pointer type is native-only and not supported on JVM target"
opPrefix (Blob _) = error "opPrefix: blob type is native-only and not supported on JVM target"
