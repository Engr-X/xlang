...
%token
    ...
    KW_PACKAGE          { t@(Lex.Ident "package" _) }
    KW_IMPORT           { t@(Lex.Ident "import" _) }
    KW_CLASSNAME        { t@(Lex.Ident "classname" _) }
    KW_JVMCLASS         { t@(Lex.Ident "JvmClass" _) }
...
#include "Parse/GrammarDecl"


ProgramEOF :: { Program }
    : EOF                               { ([], []) }
    | TopSeq EOF                        { splitTopItems (reverse $1) }
    | SepSeq EOF                        { ([], []) }
    | SepSeq TopSeq EOF                 { splitTopItems (reverse $2) }
    | TopSeq SepSeq EOF                 { splitTopItems (reverse $1) }
    | SepSeq TopSeq SepSeq EOF          { splitTopItems (reverse $2) }


TopItem :: { Either Declaration Statement }
    : Decl { Left $1 }
    | Stmt { Right $1 }


TopSeq :: { [Either Declaration Statement] }
    : TopItem            { [$1] }
    | TopSeq TopItem     { $2 : $1 }
...
splitTopItems :: [Either Declaration Statement] -> Program
splitTopItems = foldr step ([], [])
    where
        step :: Either Declaration Statement -> Program -> Program
        step one (decls, stmts) = case one of
            Left d -> (d : decls, stmts)
            Right s -> (decls, s : stmts)
}
