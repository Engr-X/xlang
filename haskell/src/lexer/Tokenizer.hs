...
eatNumber [] = ([], [])
eatNumber str@(x:xs) = let
    temp = reverse (scanl (flip (:)) [x] xs)
    len = length $ dropWhile (not . UB.isNumber . reverse) temp
    in splitAt len str
...
eatStringLiteral s =
    let (_, matched, rest) = s =~ stringLiteralPattern :: (String, String, String)
    in if null matched
        then Nothing
