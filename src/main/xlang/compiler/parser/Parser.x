...
import xlang.compiler.parser.program.ImportedSymbolAliasMaybe
...
private val IMPORTED_SYMBOL_ALIAS_MAYBE_PARSER_ID: int = 24
private val IMPORTED_SYMBOL_PARSER_ID: int = 25
...
private inline fun makeModifierListMaybe(results: pointer<ArrayList>) -> pointer<*>
{
    ...
    val modifiers: pointer<HashSet> = new HashSet(sizeof(Modifier), Modifier.compareModifier, Modifier.hashcode)
...
private inline fun makeImportedFunction(results: pointer<ArrayList>) -> pointer<*>
{
    ...
    val alias: pointer<ImportedSymbolAliasMaybe> =
        getContainerValue(results, 4) as pointer<ImportedSymbolAliasMaybe>
...
private inline fun makeImportedFunctionAliasMaybe(results: pointer<ArrayList>) -> pointer<*>
{
    ...
    return new ImportedSymbolAliasMaybe(aliasToken.text)
}
...
{
    val qualifiedName: pointer<QualifiedName> = getContainerValue(results, 1) as pointer<QualifiedName>
    val importedFunction: pointer<ImportedSymbol> = getContainerValue(results, 3) as pointer<ImportedSymbol>
...
val IMPORTED_SYMBOL_ALIAS_MAYBE_PARSER: pointer<ParserRef> = ParserRef.fromRecursiveDown(IMPORTED_SYMBOL_ALIAS_MAYBE_PARSER_ID)

val IMPORTED_SYMBOL_PARSER: pointer<ParserRef> = ParserRef.fromRecursiveDown(IMPORTED_SYMBOL_PARSER_ID)
...
private val STATEMENT_RULE0: pointer<Rule> = new Rule(new PatternList().pushRegex(Tokenizer.KW_VAR).pushRefs(new ParserRefs(VARIABLE_DEFINE_PARSER, new PatternAtom(Tokenizer.COMMA, null), false)).pushRegex(Tokenizer.TK_LINE_TERMINATOR), makeStmtFrom_VariableDefines, Rule.STARTER_ROLE, 0)
private val STATEMENT_RULE1: pointer<Rule> = new Rule(new PatternList().pushRegex(Tokenizer.KW_VAL).pushRefs(new ParserRefs(VARIABLE_DEFINE_PARSER, new PatternAtom(Tokenizer.COMMA, null), false)).pushRegex(Tokenizer.TK_LINE_TERMINATOR), makeStmtFrom_VariableDefines, Rule.STARTER_ROLE, 0)
private val STATEMENT_RULE2: pointer<Rule> = new Rule(new PatternList().pushRef(WHILE_STATEMENT_PARSER), makeStmtFrom_WhileStmt, Rule.STARTER_ROLE, 0)
private val STATEMENT_RULE3: pointer<Rule> = new Rule(new PatternList().pushRef(FOR_STATEMENT_PARSER), makeStmtFrom_ForStmt, Rule.STARTER_ROLE, 0)
...
private val STATEMENT_RULE5: pointer<Rule> = new Rule(new PatternList().pushRegex(Tokenizer.KW_BREAK).pushRegex(Tokenizer.TK_LINE_TERMINATOR), makeStmtFrom_BreakStmt, Rule.STARTER_ROLE, 0)
private val STATEMENT_RULE6: pointer<Rule> = new Rule(new PatternList().pushRegex(Tokenizer.KW_CONTINUE).pushRegex(Tokenizer.TK_LINE_TERMINATOR), makeStmtFrom_ContinueStmt, Rule.STARTER_ROLE, 0)
private val STATEMENT_RULE7: pointer<Rule> = new Rule(new PatternList().pushRegex(Tokenizer.KW_PASS).pushRegex(Tokenizer.TK_LINE_TERMINATOR), makeStmtFrom_PassStmt, Rule.STARTER_ROLE, 0)
private val STATEMENT_RULE8: pointer<Rule> = new Rule(new PatternList().pushRef(EXPRESSION_PARSER).pushRegex(Tokenizer.TK_LINE_TERMINATOR), makeStmtFrom_ExprStatement, Rule.STARTER_ROLE, 0)
private val STATEMENT_RULE9: pointer<Rule> = new Rule(new PatternList().pushRefs(new ParserRefs(EXPRESSION_PARSER, new PatternAtom(Tokenizer.COMMA, null))).pushRegex(Tokenizer.TK_LINE_TERMINATOR), makeStmtFrom_ExprListStatement, Rule.STARTER_ROLE, 0)
...
fun parseImportedSymbolAliasMaybe(input: pointer<TokenList>) -> pointer<ImportedSymbolAliasMaybe>
{
    if input == null:
        return null

    if IMPORTED_SYMBOL_ALIAS_MAYBE_PARSER.doParse(input) < 0:
        return null

    val result: pointer<ParseContainer> = IMPORTED_SYMBOL_ALIAS_MAYBE_PARSER.getResult()

    if result == null || result.isKind(IMPORTED_SYMBOL_ALIAS_MAYBE_PARSER_ID) == false:
        return null

    return result.getValue() as pointer<ImportedSymbolAliasMaybe>
}

fun parseImportedSymbol(input: pointer<TokenList>) -> pointer<ImportedSymbol>
{
    if input == null:
        return null

    if IMPORTED_SYMBOL_PARSER.doParse(input) < 0:
        return null

    val result: pointer<ParseContainer> = IMPORTED_SYMBOL_PARSER.getResult()

    if result == null || result.isKind(IMPORTED_SYMBOL_PARSER_ID) == false:
        return null

    return result.getValue() as pointer<ImportedSymbol>
}
