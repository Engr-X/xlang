...
fun filter(dest: pointer<*>, src: pointer<*>, f: (pointer<*>) -> bool, length: int, tsize: int)
{
    var ptr: pointer<byte> = src as pointer<byte>
    var destPtr: pointer<byte> = dest as pointer<byte>

    for (var offset: int = 0; offset < length; ptr += tsize, offset++):
    {
        if f(ptr):
        {
            System.memcopy(destPtr, ptr, tsize)
            destPtr += tsize
        }
    }
}
